...
PROJECT_ZIP = ROOT_DIR / "marina_pflegebox_konfigurator.zip"
...
def generate_filled_pdf(order: Order) -> bytes:
    ...
    # Field leistungserbringer_name_addr wurde aus dem Original-PDF entfernt
    ...
    # Today's date - ohne Punkte für kleine Felder
    today = datetime.now()
    date_str = today.strftime("%d%m%Y")  # TTMMJJJJ ohne Punkte
    ...
    # === PAGE 0 FIELDS ===
    # Geburtsdatum ohne Punkte (z.B. "15.01.1950" -> "15011950")
    geb_ohne_punkte = customer.geburtsdatum.replace(".", "")
    
    page0_fields = {
        "name_vorname": full_name,
        "geb_1": geb_ohne_punkte,
    ...
    # === PAGE 1 FIELDS ===
    page1_fields = {
        "leistungserbringer_name_addr": "Pflegebox by Marina\nHervesterstr. 16\n46286 Dorsten",
        ...
        # genehm_pg54 NICHT setzen - wird von Pflegekasse ausgefüllt
    }
    
    # genehm_pg54_beihilfe auch NICHT setzen
...
async def download_project():
    ...
    if PROJECT_ZIP.exists():
        return FileResponse(
            str(PROJECT_ZIP),
            media_type="application/zip",
            filename="marina_pflegebox_konfigurator.zip"
        )
...
# ============ STATIC FILE SERVING (STANDALONE) ============
